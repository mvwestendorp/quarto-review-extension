// Debug mode - checks for data-web-review-debug attribute on body element
const DEBUG = document.body.hasAttribute('data-web-review-debug');
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function makeElementEditable(element, clickCharOffset = null) {
    ...
    toolbar.style.cssText = 
      'display: flex; gap: 4px; margin-bottom: 8px; padding: 4px; background: #f8f9fa; border-radius: 3px; flex-wrap: wrap;';
    ...
    toolbarButtons.forEach((btn, index) => {
      ...
      button.className = 'web-review-toolbar-btn';
      button.style.fontWeight = btn.text === 'B' ? 'bold' : btn.text === 'I' ? 'italic' : 'normal';
      ...
      toolbarButtonElements.push(button);
      toolbar.appendChild(button);
    });
    
    // Create visual editor div - use CURRENT content for editing
    const visualEditor = document.createElement('div');
    visualEditor.contentEditable = true;
    visualEditor.className = 'web-review-visual-editor';
    visualEditor.innerHTML = convertMarkdownToHtml(currentMarkdown);
    visualEditor.style.cssText =
      'width: 100%; min-height: 80px; padding: 8px; border: 1px solid #ddd; border-radius: 3px; ' +
      'background: white; overflow-y: auto; outline: none; ' +
      'font-size: 14px !important; line-height: 1.4 !important; ' +
      'color: #333 !important; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Arial, sans-serif !important;';
    ...
    editContainer.appendChild(visualEditor);
    ...
    function insertVisualLink() {
      if (!isVisualMode) {
        return insertSourceLink();
      }
      
      const url = prompt('Enter URL:', 'https://');
      if (url) {
        const text = prompt('Enter link text:', 'link text') || url;
        visualEditor.focus();
        document.execCommand('createLink', false, url);
        
        // If no text was selected, replace with custom text
        const selection = window.getSelection();
        if (selection.toString() === url && text !== url) {
          const range = selection.getRangeAt(0);
    ...
    function addListItem() {
      ...
      if (selection.rangeCount > 0) {
        ...
        if (listItem && listItem.tagName === 'LI') {
          ...
          // Not in a list - create a new unordered list
          visualEditor.focus();
          document.execCommand('insertHTML', false, '<ul><li>New list item</li></ul>');
        }
      }
    }
    ...
    function switchToVisualMode() {
      ...
      visualEditor.innerHTML = convertMarkdownToHtml(sourceMarkdown);
    ...
    function switchToSourceMode() {
      ...
      textarea.value = convertHtmlToMarkdown(visualHtml);
    ...
    function updateToolbarState() {
      ...
      try {
        ...
        commands.forEach((command, index) => {
          ...
          toolbarButtonElements[index].classList.toggle('active', isActive);
        });
        
        // Check for code formatting (custom logic)
        const selection = window.getSelection();
        if (selection.rangeCount > 0) {
          const range = selection.getRangeAt(0);
    ...
    saveBtn.addEventListener('click', function() {
      ...
      let newMarkdown;
      if (isVisualMode) {
        newMarkdown = convertHtmlToMarkdown(visualEditor.innerHTML);
      } else {
    ...
    cancelBtn.addEventListener('click', function() {
      ...
      // Restore proper content based on element state
      if (element.classList.contains('web-review-modified')) {
        // Element was previously modified - restore to the modified diff view
        const storedOriginal = element.dataset.originalText;
        const storedNew = element.dataset.newText;
        const storedOriginalMd = element.dataset.originalMarkdown;
        const storedNewMd = element.dataset.newMarkdown;
        
        if (storedOriginal && storedNew) {
          showInlineDiff(element, storedOriginal, storedNew, storedOriginalMd, storedNewMd);
        } else {
          element.innerHTML = originalHTML;
        }
      } else {
        // Element was not previously modified - restore original HTML
        element.innerHTML = originalHTML;
      }
  ...
  function saveElementChange(element, originalMarkdown, newMarkdown) {
    ...
    // Store the change with user attribution
